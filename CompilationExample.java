...
//     CompiledProgram label_propagation = session.compileProgram("../vlab.gm");   
  System.out.println("\n After Compiling greenmarl Java Program");
   // PgxGraph graph = session.readGraphWithProperties("/var/services/homes/vavenkatesh/pgx-release-1.1.0-otn/examples/graphs/sample_lp.adj.json");
  ...
  //    PgxGraph subgraph = graph.filter(new VertexFilter("name.nodeID < 100"));
   ...
   // System.out.println(label.get(2));
    ...
    System.out.println("Total Nodes Traversed= " + result.getReturnValue() + " (took " + result.getExecutionTimeMs() + "ms)");
  }
}
